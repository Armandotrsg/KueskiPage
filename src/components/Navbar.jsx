...
export const Navbar = () => {
    ...
    const NavigationItems = ({ name, path, icon }) => {
        return (
            <li className={`flex w-full items-center justify-center md:border-r-[5px] ${path === location.pathname ? (isOpen ? "border-r-[5px] border-lime-500" : "md:border-lime-500") : "border-transparent"}`}>
                <a
                    href={path}
                ...
                >
                    ...
                    <div className={`flex flex-col w-[50%] items-start justify-center text-md lg:text-lg ${!isOpen ? "hidden" : ""}`}>
                        <span className="flex">
                            {name}
                        </span>
                    </div>
